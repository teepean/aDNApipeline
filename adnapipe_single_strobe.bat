...
bin\strobealign --rg-id=ILLUMINA-%INDNAME% --rg=SM:%INDNAME% --rg=PL:%ILLUMINA% --rg=PU:ILLUMINA-%INDNAME%-sE -t %THREADS% hs37d5.fa --interleaved %INDNAME%\%INDNAME%.fastp.fastq.gz| bin\samtools sort --no-PG -@ %THREADS% -m2G -O bam - > %INDNAME%\%INDNAME%_SE.mapped.bam
